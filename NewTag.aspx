...
<body>
                ...
                <div class="row">
                    <div class="col-4" style="width: 80%;">
                        <div style="font-family: 'Roboto', sans-serif; font-size: 14px; font-weight: 400">
                            My Personal Tags (<span id="personaltagsCount"></span>)
            ...
            <div id="dvTagGroups">
                <div class="row">
                    ...
                    <div style="padding-top: 5px; margin-left: 5px;">
                        <i class="fa fa-video fa-sm"></i>&nbsp;&nbsp;<a class="tagMgntVideoLinks" href="<%= groupTagVideoLink %>.aspx" target="_blank">How To - Group Tags</a>
                    </div>
    ...
    <script>
        ...
        function removeTag() {
            ...
            $.ajax({
                url: 'NewTag.aspx/removeTag?tgId=' + JSON.stringify($('#hdnTagId').val()) + '&toolId=' + $('#hdnToolId').val() + '&tagName=' + JSON.stringify($('#hdnTagName').val()),
                type: 'GET',
                contentType: 'application/json',
                dataType: 'json',
                success: function (data, status, jqXHR) {
        ...
        function applyTag() {
            ...
            $.ajax({
                url: 'NewTag.aspx/applyTag?tagId=' + $('#hdnTagId').val() + '&toolId=' + $('#hdnSelectedToolId').val() + '&tagName=' + JSON.stringify($('#hdnTagName').val()),
                type: 'GET',
                contentType: 'application/json',
                dataType: 'json',
                success: function (data, status, jqXHR) {
            ...
            if ($('#btnOkTaggedEntires').val() == "OK") {
                ...
                for (var j = 0; j < toolArray.length; j++) {
                    if (!document.getElementById(toolArray[j]).checked) {
                        ...
                        $.ajax({
                            url: 'NewTag.aspx/removeToolFromTag?tgId=' + $('#hdnTagId').val() + '&toolId=' + toolArray[j] + '&tagName=' + JSON.stringify($('#hdnTagName').val()),
                            type: 'GET',
                            contentType: 'application/json',
                            dataType: 'json',
                            success: function (data, status, jqXHR) {
        ...
        function addinapprTag() {
            ...
            if (lstTags.filter(x => x.TAG_LABEL.trim() === $('#txtTagName').val().trim()).length > 0) {
                ...
                $('#hdnTagsByWgId').val(JSON.stringify(lstTags));
                /* var orgList = JSON.parse($('#hdnOriginalSelections').val());
                 orgList.push(newrow);
                 $('#hdnOriginalSelections').val(JSON.stringify(orgList));*/
