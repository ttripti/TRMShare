...
<%--                            <div class="col-md-auto">
                    ...
                    <div class="modal-footer">
                        <div class="row">
                            <div class="col-12" style="text-align: center;">
                                 <button type="button" id="btnTaggedEntires" class="cancelButton" data-dismiss="modal" onclick="javascript:cancelviewTRMModel();;return false;">CANCEL</button>
                                <button type="button" id="btnOkTaggedEntries" style="padding: 10px 30px 10px 30px;" class="enabledbutton" onclick="javascript:removeTool();return false;">APPLY</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
                    ...
                    if (data != null) {
                        ...
                        if (data.d.lstTools != null) {
                            ...
                            $.each(data.d.lstTools, function (key, val) {
                                ...
                                parentDiv += '<div style="font-family: Roboto;font-size:11px;padding-left:30px;padding-bottom:5px;" class="list-group-item list-group-item-action" id="' + aid + '" ><input type="checkbox"  onchange="toolChange(this)" toolid="' + val.Tool_Id + '"';
        ...
        function trmEntryChange(element) {
            $('#btnOkTaggedEntries').removeAttr('disabled', 'disabled');
            $('#btnOkTaggedEntries').removeClass('disabledbutton').addClass('enabledbutton');
        ...
        function getToolEntries() {
            ...
            if ($('#txtToolEntry').val().trim() != "") {
                ...
                $.ajax({
                    ...
                    error: function (jqXHR, status, error) {
                        $('#spinnersearch').addClass('d-none');
                        $('#toastError').text('An Internal Error has occurred.');
                        $('#burntToast').toast('show').show().delay(3000).fadeOut();
                        //trmUtil.showSpinner(false, $('#' + config.spinnerElId)[0]);
                        //trmUtil.showHandledResponseErrors(config.containerId, jqXHR, status, error);
                    }
        ...
        function closeviewTRMModel() {
            ...
            if ($('#btnOkTaggedEntries:disabled').length == 0 && isError == '0') {
                $("#unsavedChanges").modal('show');
            }
            else {
                $('#dv' + $('#hdnTagId').val()).css('background-color', 'rgb(220, 230, 250)');
        ...
        function viewTRMEntriesModel(aTagID, isPersonal, isReopen) {
            ...
            if (isReopen != undefined && isReopen) {
                ...
                $('#btnOkTaggedEntries').attr('disabled', 'disabled');
                $('#btnOkTaggedEntries').removeClass('enabledbutton').addClass('disabledbutton');
            }
            ...
            if (RoleId < 3) {
                $('#btnOkTaggedEntries').text('APPLY');
                $('#btnOkTaggedEntries').show();
            } else {
                $('#btnOkTaggedEntries').text('OK');
                $('#btnOkTaggedEntries').show();
            }
        ...
        $(document).ready(function () {
            ...
            document.querySelector("#btnOkTaggedEntries").addEventListener('click', createRipple);
