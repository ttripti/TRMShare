...
Public Class NewTag
    ...
    Public tagManagementVideoLink As String
    Public personalTagVideoLink As String
    Public groupTagVideoLink As String
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            ...
            Dim query As String = "SELECT [LINK_ID],[LINK_URL] FROM [dbo].[TRM_EXTERNAL_LINKS] ORDER BY SORT_ORDER_ID ASC"
            ...
            Call trmCodeLib.executeQueryRS(query, trmGlobals.RS)
            Do While trmGlobals.RS.Read
                linkId = trmGlobals.RS("LINK_ID")
                If linkId = 30 Then tagManagementVideoLink = trmGlobals.RS("LINK_URL")
                If linkId = 31 Then personalTagVideoLink = trmGlobals.RS("LINK_URL")
                If linkId = 32 Then groupTagVideoLink = trmGlobals.RS("LINK_URL")
            Loop
            trmGlobals.RS.Close()
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function applyTag(tagId As Integer, toolId As Integer, tagName As String) As VAResponse
        ...
        If (tagId <> 0) Then
            ...
            If retVal = 1 Then
                response.IsValid = True
                tagService_.createTagHistory(tagId, _userId, "", tagName, toolId, "Tag Entry", tagName, "", "", "", tagName)
            ElseIf (retVal = 2) Then
                response.IsValid = False
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function removeTag(tgId As Integer, toolId As Integer, tagName As String) As VAResponse
        ...
        If (tgId <> 0) Then
            ...
            If retVal = True Then
                ...
                tagService_.createTagHistory(tgId, _userId, tagName, "", toolId, "UnTag Entry", tagName, "", "", "", tagName)
            Else
                response.IsValid = False
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function removeToolFromTag(tgId As Integer, toolId As Integer, tagName As String) As VAResponse
        ...
        If (tgId <> 0) Then
            ...
            If retVal = True Then
                ...
                tagService_.createTagHistory(tgId, _userId, tagName, "", toolId, "Remove Tool", tagName, "", "", "", tagName)
            Else
                response.IsValid = False
