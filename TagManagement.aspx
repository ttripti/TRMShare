                    ...
                    if (data != null) {
                        ...
                        if (data.d.IsValid) {
                            ...
                            $.each(templstAllTags, function (key, val) {
                                ...
                                parentDiv += '&nbsp;<div Id=' + btnId + ' tagId=' + val.TAG_ID + ' tagName="' + val.TAG_LABEL + '" class="tagellipsisbuttonpt"  style="width:7px;display: inline-block;cursor:pointer;padding-left:5px;padding-right:5px;"><i class="fas fa-ellipsis-v tag" style="color: #757575;cursor:pointer;"></i></div>'
                                parentDiv += '<div style="height:100%;display: inline-block;padding-right:10px;padding-top:6px;font-size:11px;font-family: Roboto, sans-serif;font-weight:border-radius: 3px;white-space: pre-wrap;cursor:pointer" onclick="javascript:viewTRMEntriesModel(' + false + ',' + val.TAG_ID + ',' + aNull + ',null);return false;">&nbsp;&nbsp;<i class="fas fa-user"></i>&nbsp;&nbsp;' + val.TAG_LABEL + ' (' + val.EntryCount + ')' + '</div></div></div></div>';
                                counter++;
                            ...
                            $.each(lstWgs, function (key, val) {
                                ...
                                $.each(templstAllTags, function (key, val) {
                                    ...
                                    if (wgId == val.WG_ID) {
                                        ...
                                        if (record != undefined) {
                                            tagsdiv += '<div style="display:inline-block;padding:4px 4px 4px 4px;"><div Id=' + dvId + ' style="display:flex;height:28px;background-color:rgb(220, 230, 250);border-radius:2px;padding-bottom:5px;" ><div style="font-size:12px;display:inline-block;border-radius:6px;">'
                                            tagsdiv += '&nbsp;<div RoleId=' + u.Role + ' Id=' + btnId + ' wgId=' + val.WG_ID + ' tagId=' + val.TAG_ID + ' tagName="' + val.TAG_LABEL + '" groupName="' + groupName + '" class="tagellipsisbutton"  style="width:7px;display: inline-block;cursor:pointer;padding-left:5px;padding-right:5px;"><i class="fas fa-ellipsis-v tag" style="color: #757575;cursor:pointer;"></i></div>'
                                            tagsdiv += '<div style="height:100%;display: inline-block;padding-right:10px;padding-top:6px;font-size:11px;font-family: Roboto, sans-serif;font-weight:border-radius: 3px;white-space: pre-wrap;cursor:pointer" onclick="javascript:viewTRMEntriesModel(' + true + ',' + val.TAG_ID + ',' + val.WG_ID + ',\'' + groupName + '\');return false;">&nbsp;&nbsp;<i class="fas fa-user-friends"></i>&nbsp;&nbsp;' + val.TAG_LABEL + ' (' + val.EntryCount + ')' + '</div></div></div></div>';
                                            counter++;
                                        }
                                    }
                                });
