...
<asp:Content ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="toast text-danger text-center" style="position: fixed; top: 10rem; left: 50%; z-index: 9998; background-color: lightsalmon; width: fit-content;" id="burntToast" data-autohide="false">
    ...
    <div style="max-width: 826px; min-width: 400px">
        ...
        <span id="dots">...&nbsp;</span>
            <span id="more">manner. You may use this page to view and manage your personal and group tags. Here, you may easily add TRM entries to your tags within the My Personal Tags or My Group Tags sections.<br />
                <br />
    ...
    <div id="dvTagPopup" class="messagepop tagpop shadow-sm mb-5 bg-white rounded" style="margin-top: 1px; display: none; width: 170px; border: 1px solid #dee2e6; border-radius: 5px">
        ...
        <div style="padding-top: 5px; padding-bottom: 5px; background-color: white; cursor: pointer" class="grpellipsemenu" id="dvDeleteTag" onclick="javascript:deleteTagModel2nd();return false;">
            <div style="padding-left: 10px; display: inline-block; color: rgb(80,80,80)">
                <i class="fas fa-trash"></i>
            </div>
            <div style="display: inline-block;">
    ...
    <div class="modal fade" id="deleteTagModel2nd" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document" style="width: 600px; right: 100px;">
            <div class="modal-content" style="width: 500px;">
                ...
                <div class="modal-body">
                    <div class="row" style="width: 100%;">
                        <div class="col-8">
                            <i class="fas fa-exclamation-triangle" id="DeleteWarning2nd" style="color: #EB5757;"></i><span class="textcss" style="color: black; font-weight: bold; font-size: 16px;">&nbsp; DELETE TAG</span>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-8">
                            <div id="dvdeletegrpModel2nd">
                                    <div class="group">
                                        <label style="font-family: Roboto, sans-serif; font-size: 11px; color: grey; text-align: left; padding-top: 4px;">Tag Name</label>
                                        <input type="text" id="txtDeleteTagName2nd" autocomplete="off" style="font-family: Roboto, sans-serif; font-size: 15px; width: 100%; background-color: #F6F6F6;" required="required" />
                                        <span class="highlight"></span>
                                        <span class="bar"></span>
                                    </div>
                            </div>
                ...
                <div class="modal-footer">
                    <div class="row">
                        <div class="col-12" style="text-align: center;">
                            <button type="button" id="btncanceldeletetag2nd" class="cancelButton" data-dismiss="modal">CANCEL</button>
                            <button type="button" id="btnDeleteWorkTag2nd" class="disabledbutton" disabled="disabled" onclick="javascript:deleteTagModel();return false;">
                                DELETE
                            </button>
                        </div>
                    </div>
    ...
    <script>
                        ...
                        if (data != null) {
                            ...
                            if (data.d.lstTools != null) {
                                ...
                                $.each(data.d.lstTools, function (key, val) {
                                    ...
                                    parentDiv += '<a href="#" style="font-family: Roboto;font-size:11px;" class="list-group-item list-group-item-action" id="' + aid + '" onclick="javascript:selectedTool(' + val.Tool_Id + ');return false;">' + val.Tool_Name + '</a>'
        ...
        function deleteTagModel2nd() {
            ...
            $('#deletetagpopuptext2nd').html("Type the name <q><b>&#91;" + $('#ContentPlaceHolder1_selectedTagName').val() + "&#93;</b></q> above to confirm your request to delete the <b>&#91;" + $('#ContentPlaceHolder1_selectedTagName').val() + "&#93;</b>.  Once you press Delete, the tag canot be recovered.");
            $("#deleteTagModel2nd").modal('show');

            if ($('#ContentPlaceHolder1_hdnSelecteconfirdWGId').val() == '-1')
                $('#deleteTagpopuptext2nd').html("Are you sure you want to delete the tag <b>" + $('#ContentPlaceHolder1_selectedTagName').val() + "</b>? This cannot be undone.");
            else 
                $('#deleteTagpopuptext2nd').html("Are you sure you want to delete the tag <b>" + $('#ContentPlaceHolder1_selectedTagName').val() + "</b> from the group <b>" + $('#ContentPlaceHolder1_hdnSelectedWGName').val() + "</b>? This cannot be undone.");
            ...
            if ($('#btnOkTaggedEntires').val() == "OK") {
                ...
                for (var j = 0; j < toolArray.length; j++) {
                    if (!document.getElementById(toolArray[j]).checked) {
                        numElements = j;
                        //confirm("removeToolfromTag tagid-->" + $('#ContentPlaceHolder1_selectedTagId').val() + " toolId-->" + toolArray[j])

                        $.ajax({
                            url: 'TagManagement.aspx/removeToolFromTag?tgId=' + $('#ContentPlaceHolder1_selectedTagId').val() + '&toolId=' + toolArray[j],
                            type: 'GET',
                            contentType: 'application/json',
                            dataType: 'json',
                            success: function (data, status, jqXHR) {
                    ...
                    if (data != null) {
                        ...
                        if (data.d.IsValid) {
                            ...
                            $.each(templstAllTags, function (key, val) {
                                ...
                                var aNull = null;
                                parentDiv += '<div style="display:inline-block;padding:4px 4px 4px 4px;"><div Id=' + dvId + ' style="display:flex;background-color:rgb(220, 230, 250);border-radius:2px;height:28px;padding-left:3px;" ><div style="font-size:12px;display:inline-block;border-radius:6px;">'
                                parentDiv += '<div Id=' + btnId + ' tagId=' + val.TAG_ID + ' tagName="' + val.TAG_LABEL + '" class="tagellipsisbuttonpt"  style="height:100%;width:7px;float:left;cursor:pointer;padding-left:3px;padding-right:4px;padding-top:5px;"><i class="fas fa-ellipsis-v tag" style="color: #757575;cursor:pointer;"></i></div>'
                            ...
                            $.each(lstWgs, function (key, val) {
                                ...
                                $.each(templstAllTags, function (key, val) {
                                    ...
                                    if (wgId == val.WG_ID) {
                                        ...
                                        if (record != undefined) {
                                            tagsdiv += '<div style="display:inline-block;padding:4px 4px 4px 4px;"><div Id=' + dvId + ' style="display:flex;height:28px;background-color:rgb(220, 230, 250);border-radius:2px;padding-left:3px;" ><div style="font-size:12px;display:inline-block;border-radius:6px;">'
                                            tagsdiv += '&nbsp;<div RoleId=' + u.Role + ' Id=' + btnId + ' wgId=' + val.WG_ID + ' tagId=' + val.TAG_ID + ' tagName="' + val.TAG_LABEL + '" groupName="' + groupName + '" class="tagellipsisbutton"  style="height:100%;width:7px;float:left;cursor:pointer;padding-left:3px;padding-right:4px;padding-top:5px;"><i class="fas fa-ellipsis-v tag" style="color: #757575;cursor:pointer;"></i></div>'
                                            tagsdiv += '<div id="daTagName"' + val.TAG_ID + ' style="height:100%;float:left;padding-right:10px;padding-top:7px;font-size:11px;font-family: Roboto, sans-serif;font-weight:border-radius: 3px;white-space: pre-wrap;cursor:pointer" onclick="javascript:viewTRMEntriesModel(' + true + ',' + val.TAG_ID + ',' + val.WG_ID + ',\'' + groupName + '\');return false;">&nbsp;&nbsp;'
                                            tagsdiv += '<i class="fas fa-user-friends" ></i >&nbsp;&nbsp; ' + val.TAG_LABEL + ' (' + val.EntryCount + ')' + '</div ></div ></div ></div > ';
                                            counter++;
                                        }
                                    }
                                });
        ...
        $(document).ready(function () {
            ...
            $(document).on("click", function (e) {
                ...
                if (e.target.className != undefined) {
                    ...
                    if ((e.target.className != 'fa fa-info-circle infoButton') && e.target.className != 'fa fa-info-circle emailIdInfo') {
                        var enabled = false;
                        if ($('#dvRolePopup').is(':visible') || $('#dvEmailIdInfo').is(':visible')) {
                            $('#dvRolePopup').hide();
                            $('#dvEmailIdInfo').hide();
