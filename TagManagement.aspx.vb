...
Public Class TagManagement
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function deleteMember(WgId As Integer, WgMemberId As Integer) As VAResponse
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function addNewMember(wgId As Integer, email As String) As VAResponse
        ...
        If (email <> "") Then
            ...
            If (trmCodeLib.validateVaEmailAddr(email, True)) Then
                ...
                retVal = wgService_.CreateWGMember(wgId, vauser.NetworkID, "3", vauser.FullName, _userId, email, fromEmail, toEmail, ownerfullname)
                If (retVal = 2) Then
                    response.IsValid = False
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function validateUser(email As String) As VAResponse
        ...
        If (email <> "") Then
            ...
            If (trmCodeLib.validateVaEmailAddr(email, True)) Then
                ...
                response.DisplayEmailText = vauser.FullName + "<br/>" + vauser.Email + " | " + vauser.PhoneNumber + "<br/>" + vauser.Title + " | " + vauser.Department
                'Else
                '    response.IsValid = True
                '    response.Message = "User added successfully"
                'End If
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function UpdateRoleChange(wgMemberId As Integer, wgId As Integer, roleId As Integer) As VAResponse
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function renameGroup(groupName As String, IsInAppropriateFlagged As String, wgId As Integer) As VAResponse
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function deleteGroup(wgId As Integer) As VAResponse
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function getAllUserWorkGroups() As VAResponse
        ...
        Return response
    End Function
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function renameTag(tagName As String, IsInAppropriateFlagged As String, tagId As Integer) As VAResponse
    ...
    <WebMethod(EnableSession:=True)> <ScriptMethod(UseHttpGet:=True, ResponseFormat:=ResponseFormat.Json)> Public Shared Function deleteTag(tgId As Integer) As VAResponse
